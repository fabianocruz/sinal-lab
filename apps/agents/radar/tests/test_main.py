...
Tests for apps.agents.radar.main module, ensuring proper CLI argument parsing,
...
from apps.agents.radar.main import setup_logging, main
...
class TestMainCLI:
    ...
    @patch('apps.agents.radar.main.RadarAgent')
    @patch('apps.agents.radar.main.setup_logging')
    @patch('builtins.open', new_callable=mock_open)
    @patch('sys.argv', ['main.py', '--dry-run'])
    def test_dry_run_mode_no_file_created(self, mock_file_open, mock_setup_logging, mock_radar_agent_class):
        ...
        mock_radar_agent_class.return_value = mock_agent
    ...
    def test_normal_mode_creates_output_file(
        self, mock_makedirs, mock_file_open, mock_setup_logging, mock_radar_agent_class
    ):
        """Test that normal mode creates output file with correct content."""
        # Setup mock agent
        mock_agent = Mock()
        mock_result = Mock()
        md_content = "# RADAR Report\n\nTest content"
        ...
        mock_makedirs.assert_called_once_with("apps/agents/radar/output", exist_ok=True)

        # Verify file written with correct content
        mock_file_open.assert_called_once_with(
            "apps/agents/radar/output/radar-week-8.md",
            "w",
            encoding="utf-8"
        )
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('os.makedirs')
    @patch('sys.argv', ['main.py', '--output', '/tmp/custom-report.md'])
    ...
    def test_default_week_is_current_week(self, mock_setup_logging, mock_radar_agent_class):
        ...
        # Verify RadarAgent was initialized with current week
        current_week = datetime.now().isocalendar()[1]
        mock_radar_agent_class.assert_called_once_with(week_number=current_week)

    @patch('apps.agents.radar.main.RadarAgent')
    @patch('apps.agents.radar.main.setup_logging')
    @patch('sys.argv', ['main.py', '--week', '42'])
    def test_custom_week_number(self, mock_setup_logging, mock_radar_agent_class):
        ...
        # Verify RadarAgent was initialized with week 42
        mock_radar_agent_class.assert_called_once_with(week_number=42)
    ...
    @patch('apps.agents.radar.main.setup_logging')
    @patch('sys.argv', ['main.py'])
    def test_agent_execution_error_propagates(self, mock_setup_logging, mock_radar_agent_class):
        """Test that agent execution errors propagate to caller."""
        # Setup mock agent that raises error
        mock_agent = Mock()
        mock_agent.run.side_effect = RuntimeError("Agent failed to collect data")
        ...
        assert "Agent failed to collect data" in str(exc_info.value)
    ...
    def test_file_write_error_propagates(
        self, mock_file_open, mock_makedirs, mock_setup_logging, mock_radar_agent_class
    ):
    ...
    def test_logging_messages(
        ...
        mock_setup_logging, mock_radar_agent_class
    ):
        ...
        mock_agent.provenance.summary.return_value = {'unique_sources': 4}
        ...
        assert any("Starting RADAR" in str(call) for call in info_calls)
