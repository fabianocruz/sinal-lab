...
from apps.agents.social_signals.task_queue import (
    TaskQueue,
    TaskResult,
    TaskStatus,
)
...
class TestTaskQueue:
    """Test the async task queue with concurrency control."""

    @pytest.mark.asyncio
    async def test_basic_enqueue_and_process(self) -> None:
        """Enqueue two tasks and process both successfully."""
        queue = TaskQueue(max_concurrency=5, task_timeout=5.0)

        async def _task_a() -> str:
            return "result_a"

        async def _task_b() -> str:
            return "result_b"

        await queue.enqueue("a", _task_a())
        await queue.enqueue("b", _task_b())
        assert queue.pending_count == 2

        results = await queue.process_all()
        assert len(results) == 2
        assert results["a"].status == TaskStatus.COMPLETED
        assert results["a"].data == "result_a"
        assert results["b"].status == TaskStatus.COMPLETED
        assert results["b"].data == "result_b"

    @pytest.mark.asyncio
    async def test_concurrency_limit_respected(self) -> None:
        """Tasks respect the semaphore concurrency limit."""
        max_concurrent_seen = 0
        current_concurrent = 0

        async def _tracked_task(task_id: str) -> str:
        ...
        queue = TaskQueue(max_concurrency=2, task_timeout=5.0)
        for i in range(5):
            await queue.enqueue(f"task_{i}", _tracked_task(f"task_{i}"))

        results = await queue.process_all()
        assert len(results) == 5
        assert all(r.status == TaskStatus.COMPLETED for r in results.values())
        # Concurrency should not exceed 2
        assert max_concurrent_seen <= 2

    @pytest.mark.asyncio
    async def test_task_timeout_marks_timed_out(self) -> None:
        """A task exceeding its timeout gets TIMED_OUT status."""
        queue = TaskQueue(max_concurrency=5, task_timeout=0.05)

        async def _slow() -> str:
            await asyncio.sleep(10)
            return "never"

        await queue.enqueue("slow", _slow())
        results = await queue.process_all()
        assert results["slow"].status == TaskStatus.TIMED_OUT
        assert "Timed out" in (results["slow"].error or "")

    @pytest.mark.asyncio
    async def test_task_exception_marks_failed(self) -> None:
        """A task that raises an exception gets FAILED status."""
        queue = TaskQueue(max_concurrency=5, task_timeout=5.0)

        async def _failing() -> str:
            raise ValueError("broken")

        await queue.enqueue("bad", _failing())
        results = await queue.process_all()
        assert results["bad"].status == TaskStatus.FAILED
        assert "broken" in (results["bad"].error or "")

    @pytest.mark.asyncio
    async def test_process_all_empty_returns_empty(self) -> None:
        """Processing with no enqueued tasks returns empty dict."""
        queue = TaskQueue()
        results = await queue.process_all()
        assert results == {}

    @pytest.mark.asyncio
    async def test_run_parallel_static_method(self) -> None:
        """The static run_parallel helper works end-to-end."""
        async def _coro_a() -> int:
            return 1

        async def _coro_b() -> int:
            return 2

        results = await TaskQueue.run_parallel(
            tasks=[("a", _coro_a()), ("b", _coro_b())],
            max_concurrency=3,
        )
        assert results["a"].data == 1
        assert results["b"].data == 2

    @pytest.mark.asyncio
    async def test_get_summary_counts(self) -> None:
        """get_summary returns correct counts by status."""
        queue = TaskQueue(max_concurrency=5, task_timeout=0.05)
        ...
        await queue.enqueue("ok_task", _ok())
        await queue.enqueue("fail_task", _fail())
        await queue.enqueue("slow_task", _slow())
    ...
    async def test_elapsed_seconds_tracked(self) -> None:
        """Each task result has a positive elapsed_seconds."""
        queue = TaskQueue(max_concurrency=5, task_timeout=5.0)
        ...
        await queue.enqueue("timed", _brief())
        results = await queue.process_all()
        assert results["timed"].elapsed_seconds > 0.0

    @pytest.mark.asyncio
    async def test_backend_defaults_to_memory(self) -> None:
        """Default backend is 'memory'."""
        queue = TaskQueue()
        assert queue.backend == "memory"

    @pytest.mark.asyncio
    async def test_redis_backend_falls_back_to_memory(self) -> None:
        """When redis.asyncio is not importable, falls back to memory."""
...
# ===========================================================================
# Tests for SocialSignalsAgent async integration
# ===========================================================================
