...
const EXPECTED_AGENT_KEYS: AgentKey[] = [
  "sintese",
  "radar",
  "codigo",
  "funding",
...
describe("AGENT_PERSONAS", () => {
  it("has exactly 6 entries", () => {
    expect(Object.keys(AGENT_PERSONAS)).toHaveLength(6);
  });
  ...
  it("every persona has a non-empty description", () => {
    for (const [key, persona] of Object.entries(AGENT_PERSONAS)) {
      expect(typeof persona.description, `description should be string for "${key}"`).toBe(
        "string",
      );
      expect(
        persona.description.trim().length,
        `description is empty for "${key}"`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("every persona has a non-empty avatarPath", () => {
    for (const [key, persona] of Object.entries(AGENT_PERSONAS)) {
      ...
      expect(persona.avatarPath.trim().length, `avatarPath is empty for "${key}"`).toBeGreaterThan(
        0,
      );
    }
  });
...
describe("AGENT_PERSONAS agentCode values", () => {
  ...
  it("social_signals has agentCode SOCIAL", () => {
    expect(AGENT_PERSONAS.social_signals.agentCode).toBe("SOCIAL");
  });
  ...
  it("all agentCodes are uppercase strings", () => {
    for (const [key, persona] of Object.entries(AGENT_PERSONAS)) {
      expect(persona.agentCode, `agentCode for "${key}" should be uppercase`).toBe(
        persona.agentCode.toUpperCase(),
      );
    }
  });
...
describe("AGENT_PERSONAS color values", () => {
  ...
  it("has correct color for social_signals (#FF59B4)", () => {
    expect(AGENT_PERSONAS.social_signals.color).toBe("#FF59B4");
  });
...
describe("AgentKey type", () => {
  it("AGENT_PERSONAS includes all 6 expected AgentKey values as keys", () => {
    ...
    expect(keys).toContain("social_signals");
  });
...
describe("AGENT_COLORS", () => {
  it("has exactly 6 entries", () => {
    expect(Object.keys(AGENT_COLORS)).toHaveLength(6);
  });
  ...
  it("has correct Tailwind class for social_signals", () => {
    expect(AGENT_COLORS.social_signals).toBe("agent-social_signals");
  });
});
