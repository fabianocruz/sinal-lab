...
const VALID_AGENT_KEYS: AgentKey[] = [
  "sintese",
  "radar",
  "codigo",
  "funding",
...
/** Minimal valid ContentApiItem fixture — only required fields set by default. */
function makeApiItem(overrides: Partial<ContentApiItem> = {}): ContentApiItem {
  return {
    id: "item-1",
...
describe("MOCK_NEWSLETTERS", () => {
  it("has exactly 8 items", () => {
    expect(MOCK_NEWSLETTERS).toHaveLength(8);
  });
  ...
  it("every newsletter has required fields: slug, edition, title, agent, body", () => {
    for (const newsletter of MOCK_NEWSLETTERS) {
      ...
      expect(typeof newsletter.edition, `edition not a number on slug "${newsletter.slug}"`).toBe(
        "number",
      );
  ...
  it("all valid agent keys are represented across all newsletters", () => {
    const usedAgents = new Set(MOCK_NEWSLETTERS.map((n) => n.agent));
    for (const key of VALID_AGENT_KEYS) {
      expect(usedAgents.has(key), `agent key "${key}" not used in any newsletter`).toBe(true);
    }
  });
});
...
describe("CARD_GRADIENTS", () => {
  ...
  it("all gradient values are non-empty strings", () => {
    for (const [index, value] of Object.entries(CARD_GRADIENTS)) {
      ...
      expect(value.trim().length, `CARD_GRADIENTS[${index}] should not be empty`).toBeGreaterThan(
        0,
      );
    }
  });
  ...
  it("each gradient value ends with the #2A2A32 base color", () => {
    for (const [index, value] of Object.entries(CARD_GRADIENTS)) {
      expect(value.includes("#2A2A32"), `CARD_GRADIENTS[${index}] missing base color #2A2A32`).toBe(
        true,
      );
    }
  });
...
describe("AGENT_HEX", () => {
  it("has exactly 6 entries", () => {
    expect(Object.keys(AGENT_HEX)).toHaveLength(6);
  });
  ...
  it("all values are valid 6-digit hex color strings", () => {
    for (const [key, hex] of Object.entries(AGENT_HEX)) {
      expect(HEX_REGEX.test(hex), `AGENT_HEX["${key}"] = "${hex}" is not a valid hex color`).toBe(
        true,
      );
    }
  });
  ...
  it("has correct hex value for social_signals (#FF59B4)", () => {
    expect(AGENT_HEX.social_signals).toBe("#FF59B4");
  });
...
describe("mapApiToNewsletter", () => {
  it("maps a social_signals item to the social_signals agent (not sintese)", () => {
    const item = makeApiItem({ agent_name: "social_signals" });

    const newsletter = mapApiToNewsletter(item);

    expect(newsletter.agent).toBe("social_signals");
    expect(newsletter.agentLabel).toBe("SOCIAL");
  });

  it("maps a known agent_name (radar) to the matching agent and agentCode label", () => {
    const item = makeApiItem({ agent_name: "radar" });

    const newsletter = mapApiToNewsletter(item);

    expect(newsletter.agent).toBe("radar");
    expect(newsletter.agentLabel).toBe("RADAR");
  });

  it("falls back to sintese when agent_name is missing", () => {
    const item = makeApiItem({ agent_name: undefined });

    const newsletter = mapApiToNewsletter(item);

    expect(newsletter.agent).toBe("sintese");
    expect(newsletter.agentLabel).toBe("SINTESE");
  });

  it("falls back to sintese when agent_name is an unrecognized value (known latent issue)", () => {
    // Documents the current, intentionally-unchanged fallback behaviour: an
    // agent_name that isn't in AGENT_PERSONAS still borrows the sintese
    // persona (name, role, avatar, color) rather than rendering neutrally.
    const item = makeApiItem({ agent_name: "some_future_agent_not_yet_mapped" });
