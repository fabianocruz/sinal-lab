...
describe('Hero', () => {
  ...
  it('renders the link to last briefing', () => {
    render(<Hero />);
    expect(screen.getByRole('link', { name: /Ou comece pelo último Briefing/i })).toBeInTheDocument();
  });
...
describe('HowItWorks', () => {
  ...
  it('renders all 6 pipeline step titles', () => {
    render(<HowItWorks />);
    const titles = ['Pesquisa', 'Validação', 'Verificação', 'Detecção de viés', 'Síntese', 'Revisão humana'];
...
describe('WaitlistForm', () => {
  ...
  it('disables input and button during loading state', async () => {
    ...
    vi.mocked(submitWaitlist).mockImplementationOnce(
      () => new Promise(() => {})
    );
