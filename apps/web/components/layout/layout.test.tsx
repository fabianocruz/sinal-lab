...
describe("Footer", () => {
  ...
  describe("footer links", () => {
    ...
    it("test_footer_briefing_semanal_points_to_newsletter", () => {
      render(<Footer />);
      expect(screen.getByRole("link", { name: "Briefing Semanal" })).toHaveAttribute(
        "href",
        "/newsletter",
      );
    });
  ...
  describe("bottom bar", () => {
    it("test_footer_renders_transparency_tagline", () => {
      render(<Footer />);
      expect(
        screen.getByText("Transparência radical. Metodologia aberta. Dados verificáveis."),
      ).toBeInTheDocument();
    });
    ...
    it("test_footer_renders_copyright_with_current_year", () => {
      ...
      const copyright = screen.getByText(
        (content) => content.includes(year) && content.includes("Sinal.lab"),
      );
