...
describe("sitemap", () => {
  ...
  it("test_sitemap_contains_4_static_pages", async () => {
    const { default: sitemap } = await import("./sitemap");
    const result = sitemap();
    const staticUrls = result.filter((entry) => !entry.url.includes("/newsletter/"));
    expect(staticUrls).toHaveLength(4);
  });
  ...
  it("test_sitemap_contains_all_newsletter_slugs", async () => {
    ...
    expect(newsletterUrls).toHaveLength(MOCK_NEWSLETTERS.length);
  });

  it("test_sitemap_total_is_4_plus_newsletters", async () => {
    const { default: sitemap } = await import("./sitemap");
    const result = sitemap();
    expect(result).toHaveLength(4 + MOCK_NEWSLETTERS.length);
  });
  ...
  it("test_sitemap_newsletter_urls_contain_slugs", async () => {
    const { default: sitemap } = await import("./sitemap");
    const result = sitemap();
    MOCK_NEWSLETTERS.forEach((newsletter) => {
